...
{
    public class CreatePostHandler : IRequestHandler<CreatePostCommand, CreatePostOutput>
    {
        ...
        public CreatePostHandler(IMessagesHandler messagesHandler,
        ...
        public async Task<CreatePostOutput> Handle(CreatePostCommand command, CancellationToken cancellationToken)
        {
            if (command.IsValid())
            {
                ...
                loggedInUser.Posts.Add(newPost);

                await _userRepository.CommitTransactionAsync();

                await _notificationRealTime.NotifyAsync($"Novo post criado '{newPost.Title}'. em {DateTime.Now:dd/MM/yyyy' as 'HH:mm:ss}", newPost.Id.ToString());
